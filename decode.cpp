...
ALU_Ops decode(const ALU_Type data) {
   if (data.instr.lsli.op == ALU_LSLI_OP) {
      ...
      if (opts.instrs) {
         cout << "lsls r" << data.instr.lsli.rd << ", r" << data.instr.lsli.rm << ", #" << setbase(10)
              << data.instr.lsli.imm << endl;
      }
      return ALU_LSLI;
      ...
      // complete
      if (opts.instrs) {
         cout << "adds r" << data.instr.add3i.rd << ", r" << data.instr.add3i.rn << ", #" << setbase(10)
              << data.instr.add3i.imm << endl;
      }
      return ALU_ADD3I;
   } else if (data.instr.sub3i.op == ALU_SUB3I_OP) {
      // 315: insert code here to print subs instruction
      if (opts.instrs) {
         cout << "subs r" << data.instr.sub3i.rd << ", r" << data.instr.sub3i.rn << ", #" << setbase(10)
              << data.instr.sub3i.imm << endl;
      }
      return ALU_SUB3I;
      ...
      if (opts.instrs) {
         ...
         if (data.instr.mov.d) {
            ...
            if (data.instr.mov.rd == 5) {
               ...
               cout << " r" << setbase(10) << (8 + data.instr.mov.rd) << ", r" << setbase(10) << data.instr.mov.rm
                    << endl;
            }
         }
      ...
      if (opts.instrs) {
         ...
         if (data.instr.add.d) {
            ...
            if (data.instr.add.rd == 5) {
               ...
               cout << " r" << setbase(10) << (8 + data.instr.add.rd) << ", r" << setbase(10) << (8 + data.instr.add.rd) << ", r" << setbase(10) << data.instr.add.rm
                    << endl;
            }
         }
            // another stack pointer case
         else if (data.instr.add.rm == 13) {
            cout << " r" << setbase(10) << data.instr.add.rd << ", r" << setbase(10) << data.instr.add.rd << ", sp" << endl;
         } else {
      ...
      if (opts.instrs) {
         ...
         if (data.instr.cmp.d) {
            ...
            if (data.instr.cmp.rd == 5) {
               ...
               cout << " r" << setbase(10) << (8 + data.instr.cmp.rd) << ", r" << setbase(10) << data.instr.cmp.rm
                    << endl;
            }
         }
            // another stack pointer case
...
LD_ST_Ops decode(const LD_ST_Type data) {
   if (data.instr.class_type.opA == LD_ST_REG_OPA) {
      if (data.instr.class_type.opB == LD_ST_OPB_LDRB) {
         ...
         if (opts.instrs) {
            cout << "ldrb r" << data.instr.ld_st_reg.rt << ", [r" << data.instr.ld_st_reg.rn << ", r"
                 << data.instr.ld_st_reg.rm << "]" << endl;
         }
         return LDRBR;
      } else if (data.instr.class_type.opB == LD_ST_OPB_STRB) {
         // 315: write code to print strb
         if (opts.instrs) {
            cout << "strb r" << data.instr.ld_st_reg.rt << ", [r" << data.instr.ld_st_reg.rn << ", r"
                 << data.instr.ld_st_reg.rm << "]" << endl;
         }
         return STRBR;
         ...
         if (opts.instrs) {
            cout << "ldr r" << data.instr.ld_st_reg.rt << ", [r" << data.instr.ld_st_reg.rn << ", r"
                 << data.instr.ld_st_reg.rm << "]" << endl;
         }
         return LDRR;
         ...
         if (opts.instrs) {
            cout << "str r" << data.instr.ld_st_reg.rt << ", [r" << data.instr.ld_st_reg.rn << ", r"
                 << data.instr.ld_st_reg.rm << "]" << endl;
         }
         return STRR;
      }
      ...
      if (data.instr.class_type.opB == LD_ST_OPB_STR) {
         if (opts.instrs) {
            cout << "str r" << data.instr.ld_st_imm.rt << ", [r" << data.instr.ld_st_imm.rn << ", #" << setbase(10)
                 << (data.instr.ld_st_imm.imm * 4) << "]" << endl;
         }
         return STRI;
      } else if (data.instr.class_type.opB == LD_ST_OPB_LDR) {
         if (opts.instrs) {
            cout << "ldr r" << data.instr.ld_st_imm.rt << ", [r" << data.instr.ld_st_imm.rn << ", #" << setbase(10)
                 << (data.instr.ld_st_imm.imm * 4) << "]" << endl;
         }
         return LDRI;
      }
      ...
      if (data.instr.ld_st_imm.op == LD_ST_LDB) {
         // 315: write code to print ldrb
         if (opts.instrs) {
            cout << "ldrb r" << data.instr.ld_st_imm.rt << ", [r" << data.instr.ld_st_imm.rn << ", #" << setbase(10)
                 << (data.instr.ld_st_imm.imm * 4) << "]" << endl;
         }
         return LDRBI;
         ...
         // 315: write code to print strb
         if (opts.instrs) {
            cout << "strb r" << data.instr.ld_st_imm.rt << ", [r" << data.instr.ld_st_imm.rn << ", #" << setbase(10)
                 << (data.instr.ld_st_imm.imm * 4) << "]" << endl;
         }
         return STRBI;
      }
      ...
      if (data.instr.ld_st_sp_imm.opB) {
         if (opts.instrs) {
            cout << "ldr r" << data.instr.ld_st_sp_imm.rt << ", [sp, #" << setbase(10) << data.instr.ld_st_sp_imm.imm
                 << "]" << endl;
            return LDRSP;
         }
      } else {
         if (opts.instrs) {
            cout << "str r" << data.instr.ld_st_sp_imm.rt << ", [sp, #" << setbase(10) << data.instr.ld_st_sp_imm.imm
                 << "]" << endl;
            return STRSP;
         }
      }
...
int decode(const LDRL_Type data) {
   ...
   if (opts.instrs) {
      cout << "ldr r" << data.instr.ldrl.rt << ", [pc, #" << setbase(10)
           << (data.instr.ldrl.imm * 4) << "]" << endl;
   }
   return LDRL;
}
